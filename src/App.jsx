...
export default function App() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      if (p >= 100) {
        p = 100
        clearInterval(iv)
        setTimeout(() => setLoaderOut(true), 80)
        setTimeout(() => setLoaderHidden(true), 380)
      }
  ...
  useEffect(() => {
    ...
    const pts = Array.from({ length: isMobile ? 28 : 110 }, () => ({
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        ctx.fillStyle = `rgba(197,164,74,${p.o * 0.5})`
        ctx.fill()
      })
    ...
    <>
      ...
      {!loaderHidden && (
        <div id="loader" className={loaderOut ? 'out' : ''}>
          ...
          <div className="l-bar">
            <div className="l-fill" style={{ width: `${loadPct}%` }} />
          </div>
        </div>
      )}
      ...
      <div className="mqs" aria-hidden="true">
        <div className="mqr">
          {[...marqueeRow1, ...marqueeRow1].map((t, i) => <span className="mqi" key={i}>{t}<span className="mqd" /></span>)}
        </div>

        <div className="mqr">
          {[...marqueeRow2, ...marqueeRow2].map((t, i) => <span className="mqi" key={i}>{t}<span className="mqd" /></span>)}
        </div>
      </div>
            ...
            <div className="sc-wrapper" key={s.num}>
              <div className={`sc-flipper ${flippedCard === s.num ? 'flipped' : ''}`}>
                <div className="sc-front">
                  ...
                  <button className="sc-arr" aria-label={`Learn more about ${s.name}`} onClick={() => setFlippedCard(flippedCard === s.num ? null : s.num)}>
                    →
                  </button>
                </div>
        ...
        <div className="pc-grid">
          {pricingTiers.map((t, i) => (
            <div className={`pc ${t.featured ? 'feat' : ''} rv d${i + 1} tilt`} key={t.name}>
              ...
              <a href="#contact" className={t.featured ? 'btn-g' : 'btn-o'} onClick={e => handleNav(e, '#contact')} style={{ textAlign: 'center', width: '100%', display: 'block', padding: 14 }}>
                Get Started
              </a>
            </div>
          ))}
