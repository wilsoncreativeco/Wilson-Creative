...
export default async function handler(req, res) {
  ...
  // ── Deposit paid → send welcome pack ────────────────────────────────────────
  if (type === 'deposit') {
          ...
          html: `
            <div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#0f0e13;border:1px solid #1e1e1e;border-radius:8px;overflow:hidden;">
              ...
              <div style="padding:28px 32px;">
                ...
                <p style="margin:0 0 20px;font-size:13px;color:#888;">Stripe confirmed. Welcome pack sent automatically.</p>
  ...
  // ── Balance paid → send launch email + handover PDF ─────────────────────────
  if (type === 'balance') {
    const firstName = (meta.client_name || '').split(' ')[0] || 'there'
    const business  = meta.business  || ''
    ...
    let pdfBuffer
    try {
      pdfBuffer = await buildHandoverPDF({ clientName: meta.client_name, business, siteUrl, pages, features })
    } catch (err) {
      console.error('Handover PDF error:', err)
    }

    const pdfBase64  = pdfBuffer?.toString('base64')
    const pdfFilename = `${business} — Site Handover.pdf`
    const amountFmt   = session.amount_total
      ? `$${(session.amount_total / 100).toLocaleString('en-AU')}`
      : 'balance'
    ...
    try {
      await Promise.all([
        resend.emails.send({
          from:        'George — Wilson Creative Co. <contact@wilsoncreativeco.au>',
          to:          email,
          subject:     `You're live, ${firstName}. Welcome to the web. ✦`,
          html:        buildLaunchEmail({ firstName, business, siteUrl }),
          attachments: pdfBase64 ? [{ filename: pdfFilename, content: pdfBase64 }] : [],
        }),
        resend.emails.send({
          from:    'Wilson Creative Co <contact@wilsoncreativeco.au>',
          to:      'wilsoncreativeco.au@gmail.com',
          subject: `🚀 Balance paid — ${meta.client_name} (${business}) is live · ${amountFmt}`,
          html: `
            <div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#0f0e13;border:1px solid #1e1e1e;border-radius:8px;overflow:hidden;">
              ${darkHeader}
              <div style="padding:28px 32px;">
                <h2 style="margin:0 0 6px;font-size:20px;color:#f0ece2;">Balance Paid — Project Complete 🚀</h2>
                <p style="margin:0 0 20px;font-size:13px;color:#888;">Launch email + handover PDF sent to client.</p>
                <table style="width:100%;border-collapse:collapse;">
                  <tr><td style="padding:10px 0;border-bottom:1px solid #1e1e1e;color:#888;width:110px;font-size:13px;">Client</td><td style="padding:10px 0;border-bottom:1px solid #1e1e1e;color:#f0ece2;font-size:13px;">${meta.client_name}</td></tr>
                  <tr><td style="padding:10px 0;border-bottom:1px solid #1e1e1e;color:#888;font-size:13px;">Business</td><td style="padding:10px 0;border-bottom:1px solid #1e1e1e;color:#f0ece2;font-size:13px;">${business}</td></tr>
                  <tr><td style="padding:10px 0;border-bottom:1px solid #1e1e1e;color:#888;font-size:13px;">Amount</td><td style="padding:10px 0;border-bottom:1px solid #1e1e1e;color:#f0ece2;font-size:13px;font-weight:600;">${amountFmt}</td></tr>
                  ${siteUrl ? `<tr><td style="padding:10px 0;color:#888;font-size:13px;">Live URL</td><td style="padding:10px 0;color:#c5a44a;font-size:13px;"><a href="${siteUrl}" style="color:#c5a44a;">${siteUrl}</a></td></tr>` : ''}
                </table>
              </div>
              ${darkFooter}
            </div>`,
      ...
      return res.status(500).json({ error: 'Failed to send launch email' })
    }
  }
